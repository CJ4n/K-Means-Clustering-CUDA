...
void KMeansOneIterationGpu(DataPoints *points, DataPoints *centroids)
{
	...
	InitPointsWithCentroidsIds<<<num_block_init_id, num_threads_inti_id, 0>>>(reduced_points, num_clusters, num_reduced_points);
	...
	ReduceFeature(points, ids_count, num_features, num_clusters, N, 1, &num_threads, &num_blocks, num_threads);
	...
	while (num_blocks * num_clusters > num_threads * 2)
	{
		...
		ReduceFeature(reduced_points, ids_count, num_features, num_clusters, N, 0, &num_threads, &num_blocks, num_threads);
		// timer_compute_centroids->Stop();
		// timer_compute_centroids->Elapsed();
	...
	if (num_blocks > 1) // if may happen, that last reduced reduced all the point, happen when num_blocks==1
	{
		...
		// 															reduced_points->cluster_id_of_point, reduced_points->features_array,
		// 															0, ids_count, N, num_features, num_clusters);
		ReduceFeature(reduced_points, ids_count, num_features, num_clusters, N, 0, &num_threads, &num_blocks, std::ceil(N / 2.0));
		// timer_compute_centroids->Stop();
