				   ...
				   int N, CountType count_in, int *num_th, int *num_bl)
{
	...
	*num_bl = num_blocks;
	...
	// 	cudaCheckError();
	// }
	// cudaMemset(ids_count,0,num_blocks * num_clusters);
	...
	// dalczego liczba featurewo wpływa na liczbe punktow w reduceanch?????
	// jaies problmey z pamiecia dzielona?
	cudaCheckError();
...
void KMeansOneIterationGpu(DataPoints *points, DataPoints *centroids)
{
	...
	// if (DEBUG)
	// {
	// 	// debugFunction(points, debug,ids_count, num_features, num_clusters, num_blocks, num_threads, num_clusters * num_blocks, "BEFORE WHILE REDUCE");
	// }
